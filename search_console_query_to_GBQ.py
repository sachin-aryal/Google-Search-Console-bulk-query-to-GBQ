...
project_id = 'comparepower.com:api-project-972049219239'  # Your Google Cloud project ID.
pkey = 'ComparePower-45ed3c72ee72.json'  # see readme for help with creating the credentials and .json key.
my_table = 'SearchConsole.GSC_FULL_PAGESLIST_25k'  # see notes in the readme with regard to creating this table before running the script.
import_action = 'append'  # 'replace' will overwrite the table if it exists, 'fail' will not overwrite the table if it exists.
PAGE_OUTPUT_FILE = "output.csv"

WEBMASTER_CREDENTIALS_FILE_PATH = "webmaster_credentials.dat"

credentials = service_account.Credentials.from_service_account_file(pkey)
...
def parse_command_line_options():
    ...
    parser.add_argument('property_uri', type=str,
                        help='The property URI to query. Must exactly match a property URI in Google Search Console')
    parser.add_argument('start_date', type=str,
                        help='The start date for the query. Should not be more than 90 days ago')
    parser.add_argument('end_date', type=str, help='The last date to query. Should not be sooner than two days ago.')
    parser.add_argument('--secrets_file', type=str, default='ComparePower-45ed3c72ee72.json',
                        help='File path of your Google Client ID and Client Secret')
    parser.add_argument('--config_file', type=str,
                        help='File path of a config file containing settings for this Search Console property.')
    ...
    parser.add_argument('--max-rows-per-day', '-n', type=int, default=25000,
                        help='The maximum number of rows to return for each day in the range')

    filters = parser.add_argument_group('filters')
    filters.add_argument('--page_filters_file', type=str, help='File path of a CSV list of pages to filter by',
                         default="pages.csv")
    filters.add_argument('--devices', nargs='*', type=str,
                         help='List of devices to filter by. By default we do segment by device.',
...
def main():
    ...
    if args.page_filters_file:
        try:
            ...
            logger.error("%s is not a valid file path", args.page_filters_file)
            sys.exit(err)
        except ValueError as err:
            logger.error("Error: all page filters must include the full URL of the Google Search Console property.")
            sys.exit(err)
    else:
        pages = []
    ...
    for day in date_range(start_date, end_date):
        ...
        output_rows = []
        print(
            "Processsing csv file with {0} pages;start_date={1};end_date={2}".format(len(pages), start_date, end_date))
        ...
        for filter_set in generate_filters(page=pages, device=args.devices, country=args.countries):
            ...
            logger.info("Request: {request};start_date={start_date};end_date={end_date}".format(request=request,
                                                                                                start_date=start_date,
                                                                                                end_date=end_date))
            response = execute_request(service, args.property_uri, request)

            if response is None:
                logger.error("Request failed %s", json.dumps(request, indent=2))
                continue
            ...
            if 'rows' in response:
                ...
                logger.info("response reveived with {number_of_rows}".format(number_of_rows=len(response['rows'])))
        ...
        with open(output_file, 'w', newline="", encoding="utf-8-sig") as file_handle:
            ...
            df = pd.DataFrame(output_rows,
                              columns=['Date', 'Query', 'Page', 'Clicks', 'Impressions', 'CTR', 'AVG_position',
                                       'Property', 'Country', 'Device', 'Website'])
            # Push this data into BigQuery
            if len(df) > 0:
                gbq.to_gbq(df, my_table, project_id=project_id, if_exists=import_action)

        with open(PAGE_OUTPUT_FILE, "a", newline="", encoding="utf-8-sig") as output_file_handler:
            csvwriter = csv.writer(output_file_handler)
            statinfo = os.stat(output_file)
            file_size = statinfo.st_size
            response_output = [len(pages), day, file_size, len(output_rows)]
            csvwriter.writerow(response_output)

        logger.info("Query for %s complete", day)
